...
var Utils_default = new class Utils {
  ...
  async getSHA(data) {
    if (!data)
      return null;
    let sha = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(data));
...
var Storage = class {
    ...
    await this.fsVault.iterate(async (item) => {
      ...
      if (meta.type === "folder") {
        ...
        if (mtime === false)
          return;
        meta.mtime = mtime;
        if (stored && stored.mtime === mtime)
          return;
      } else {
        if (stored && stored.mtime === stat.mtime + offset)
          return;
        try {
          const isBinary = Utils_default.isBinary(item.path);
          const data = isBinary ? await this.fsVault.read(item.path, true) : await this.fsVault.read(item.path);
          meta.sha1 = isBinary ? await Utils_default.getSHABinary(data) : await Utils_default.getSHA(data);
          meta.mtime = stat.mtime + offset;
        } catch (e) {
  ...
  async updatePlugin(files) {
    for (let item of files) {
      ...
      if (path.includes("..") || path.startsWith("/")) {
        console.error("Rejected suspicious update path:", item.path);
        continue;
      }
  ...
  async getFolderMTime(file, offset = 0) {
    ...
    return hasValue ? maxMtime : false;
  }
};
...
var XTimeouts = class {
  ...
  set(key, timeout, callback) {
    ...
    this.timeouts[key] = setTimeout(this.callbacks[key], timeout);
  }
  clear(key) {
...
var XSync = class {
  ...
  listVersionHistory(path, callback) {
  ...
  readVersionHistory(path, timestamp, callback) {
    if (!this.connectionOK())
      return;
    this.anysocket.send({
      type: "file_history",
  ...
  listFilesHistory(deletedOnly, callback) {
    if (!this.connectionOK())
      return;
    this.anysocket.send({
      type: "file_history",
  ...
  async processLocalEvent(action, file, args, forceChanged = false) {
    var _a;
    if ((_a = this.exclusionFilter) == null ? void 0 : _a.isExcluded(file.path))
      return;
  ...
  async onFileData(data, peer) {
    ...
    if (data.type == "send") {
      ...
      if ((_a = this.exclusionFilter) == null ? void 0 : _a.isExcluded(data.path))
        return true;
